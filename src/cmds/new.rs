...
pub fn cmd_new(path: PathBuf, bin: bool, lib: bool) -> Result<(), String> {
    let flags = [bin, lib].into_iter().filter(|&v| v).collect::<Vec<_>>();

    if flags.len() > 1 {
        return Err("You need to choose only one package template kind: bin or lib".to_string());
    }
...
pub fn create_bin_package(package_name: &str, path: PathBuf) -> Result<(), String> {
...
pub fn create_lib_package(package_name: &str, path: PathBuf) -> Result<(), String> {
